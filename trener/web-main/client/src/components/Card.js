...
const Card = ({ data, setErrorMessage, setSuccessMessage, Trainings, setTrainings, AllExercises }) => {
  ...
  const handleMoveExerciseToAnotherTraining = (exerciseID, selectedTrainingID) => {
    ...
    const targetTraining = Trainings.find((training) => training.training_id === parseInt(selectedTrainingID));
    ...
    const targetTotalTime = targetTraining.Exercise.reduce((sum, exercise) => sum + (exercise.time || 0), 0);
    ...
    if (selectedDifficulty > allowedDifficulty) {
      setErrorMessage(`Ошибка: сложность упражнения (${selectedDifficulty}) превышает уровень сложности целевой тренировки (${allowedDifficulty}).`);
      return;
    }
  ...
  const handleMove = (exerciseID, newTrainingID) => {
    ...
    const targetTraining = Trainings.find((training) => training.training_id === parseInt(newTrainingID));
    if (!targetTraining) {
      setErrorMessage('Ошибка: выбранный вариант тренировки не существует.');
      return;
    }

    const currentTotalTime = targetTraining.Exercise.reduce((sum, exercise) => sum + (exercise.time || 0), 0);
    const newTotalTime = currentTotalTime + selectedExercise.time;

    if (newTotalTime > targetTraining.time) {
      setErrorMessage(`Ошибка: общее время упражнений (${newTotalTime} мин.) превышает длительность тренировки (${targetTraining.time} мин.).`);
      return;
    }
  ...
  const handleSubmit = async (event) => {
    ...
    const selectedExercise = AllExercises.find((exercise) => exercise.id === parseInt(selectedExercises.Tag));
    ...
    const currentComplexity = Exercise.reduce((sum, exercise) => sum + (exercise.complexity || 0), 0);
    const newComplexity = parseInt(currentComplexity) + parseInt(selectedExercise.complexity);

    if (newComplexity > 10) {
      setErrorMessage(`Ошибка: суммарная сложность упражнений (${newComplexity}) превышает допустимое значение (10).`);
      setSuccessMessage('');
      return;
    }

    const currentTotalTime = Exercise.reduce((sum, exercise) => sum + (exercise.exercise_time || 0), 0);
    const newTotalTime = parseInt(currentTotalTime) + parseInt(selectedExercise.time);
    console.log(selectedExercise);
    if (newTotalTime > time) {
      setErrorMessage(`Ошибка: общее время упражнений (${newTotalTime} мин.) превышает длительность тренировки (${time} мин.).`);
      setSuccessMessage('');
      return;
    }

    try {
      const response = await fetch(`http://localhost:3001/api/AddExerciseToTraining/${training_id}`, {
        method: 'POST',
        headers: {
              ...
              <div className="exercise-header" style={{ display: 'flex', justifyContent: 'space-between' }}>
                ...
                <div className="exercise-actions">
                  <Button
                    ...
                    variant="contained"
                    title="Удалить"
                    sx={{ padding: '6px 12px', fontSize: '12px' }} // Smaller button size
                  >
                    Удалить
                  </Button>
                  <Button
                    onClick={() => handleMoveExerciseToAnotherTraining(exercise.exercise_id, selectedTrainingID)}
      ...
      <form onSubmit={handleSubmit} className="add-exercise-form" style={{ marginTop: '16px' }}>
        <Box sx={{ display: 'flex', justifyContent: 'center', flexDirection: 'column', gap: 2, maxWidth: 300, margin: '0 auto' }}>
          ...
          <Button
            ...
            variant="contained"
            color="primary"
            sx={{ marginTop: '8px', padding: '6px 12px', fontSize: '12px' }} // Smaller button size
